...
let diceRow1Arr = Array.from(document.querySelectorAll('#playerDiceRow1 .player-dice'));
let diceRow2Arr = Array.from(document.querySelectorAll('#playerDiceRow2 .player-dice'));
...
function completeMove(move) {
    ...
    totalScore += scoreval;
    document.getElementById('userScore').innerHTML = `${totalScore} points`;

    // Send the new score to the opponent if multiplayer
    if (socket !== 'undefined') {
...
function updateDice(rollBtn, dice) {
    ...
    for (let i = 0; i < 5; i++) {
        if (dice[i].checked) {
            currentRolls.push(parseInt(dice[i].nextElementSibling.nextElementSibling.value));
...
function updateButtons() {
    let moveList = ['ones', 'twos', 'threes', 'fours', 'fives', 'sixes', 'three_of_a_kind', 'four_of_a_kind', 'full_house', 'small_straight', 'large_straight', 'chance', 'yacht_z'];
...
function toggleElements(button) {
    if (btnsAndDiceShowing && button != 'roll') {
        ...
        scoringButtons.forEach(button => { button.style = 'visibility: hidden !important'; });
        dice.forEach(die => { die.style = 'visibility: hidden !important'; });
        btnsAndDiceShowing = !btnsAndDiceShowing;
    } else if (!btnsAndDiceShowing) {
        // Show all of the dice and buttons
        scoringButtons.forEach(button => { button.style = 'visibility: visible !important'; });
        dice.forEach(die => { die.style = 'visibility: visible !important'; });
        btnsAndDiceShowing = !btnsAndDiceShowing;
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    scoringButtons.forEach(button => {
        button.addEventListener('click', function () { completeMove(button.name) });
    });
